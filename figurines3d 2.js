/* Azeroth Chess V8: genuine real-time WebGL meshes; no sprites or external dependencies. */
...
function hero(side,type,c,r){
 ...
 for(let i=0;i<out.length;i+=9){let x=out[i],z=out[i+2],nx=out[i+3],nz=out[i+5];verts.push(x*co-z*si+c-3.5,out[i+1],x*si+z*co+r-3.5,nx*co-nz*si,out[i+4],nx*si+nz*co,...out.slice(i+6,i+9))}
}
