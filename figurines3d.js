/* Azeroth Chess V11: fixed homogeneous projection and centered cell-safe figures. */
...
function hero(side,type,c,r){
 ...
 for(let i=0;i<out.length;i+=9){let x=out[i],z=out[i+2],nx=out[i+3],nz=out[i+5];verts.push((x*co-z*si)*0.55+c-3.5,out[i+1],(x*si+z*co)*0.42+r-3.5,nx*co-nz*si,out[i+4],nx*si+nz*co,...out.slice(i+6,i+9))}
}
